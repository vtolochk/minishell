...
static void change_list(char *argv, char **name)
{
	...
	if (find)
	{
		find->value = ft_strsub(str, ft_strlen(*name), ft_strlen(str) - ft_strlen(*name));
		ft_strdel(name);
	}
	else
	{
		new_var = new_node();
		new_var->name = *name;
		new_var->value = ft_strsub(str, ft_strlen(*name), ft_strlen(str) - ft_strlen(*name));
...
int bi_setenv(char **argv)
{
	...
	else
	{
		...
		ft_strequ(name, "SHLVL"))
		{
			ft_printf("The following variables cannot be changed: PWD, SHLVL, OLDPWD.\n");
			ft_strdel(&name);
			return (FAIL);
		}
