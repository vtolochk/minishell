...
/*   Updated: 2018/03/30 15:49:00 by vtolochk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void create_new_str(int len, char *find, char *first_part, char **string)
{
	char *temp[2];
	char *str_find;
	int len_name;
	...
	while (len-- > 0)
	{
		...
		while (list)
		{
			if ((str_find = (ft_strnstr(find, list->name, (len_name = ft_strlen(list->name))))))
			{
				temp[0] = ft_strdup(list->value);
				temp[1] = temp[0];
				str_find = str_find + len_name;
				*string = ft_strjoin(first_part, ++(temp[0]));
				ft_strdel(&temp[1]);
				temp[1] = *string;
